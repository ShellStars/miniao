...
from .models import Subjectarticle, Subjectclass
...
def article_detail(request, column, pk):
    ...
    article = Subjectarticle.objects.filter(pk=pk, column=column, published=True)
    classes = Subjectclass.objects.all()
    relate = Subjectarticle.objects.filter(column=column, published=True)[0:2]
    pre_article = Subjectarticle.objects.filter(id__lt=pk, published=True)
    if pre_article:
        pre_article = pre_article.order_by('-id')[0]
    else:
        pre_article = Subjectarticle.objects.filter(id=pk, published=True)[0]
    next_article = Subjectarticle.objects.filter(id__gt=pk, published=True)
    if next_article:
        next_article = next_article[0]
    else:
        next_article = Subjectarticle.objects.filter(published=True)[0]
        # next_article = Subjectarticle.objects.filter(id=pk, published=True)[0]
    belong = {'name': '优势学科', 'slug': 'subject'}
    column_tmp = Subjectclass.objects.filter(slug=column)[0].name
...
def article(request, column):
    ...
    article = Subjectarticle.objects.filter(column=column, published=True).order_by("-id")
    classes = Subjectclass.objects.all()
    belong = 'subject'
