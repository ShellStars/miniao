...
def article_detail(request, column, pk):
    ...
    relate = Artiarticle.objects.filter(column=column, published=True)[0:2]
    pre_article = Artiarticle.objects.filter(id__lt=pk, published=True)
    if pre_article:
        pre_article = pre_article.order_by('-id')[0]
    else:
        pre_article = Artiarticle.objects.filter(id=pk, published=True)[0]
    next_article = Artiarticle.objects.filter(id__gt=pk, published=True)
    if next_article:
        next_article = next_article[0]
    else:
        next_article = Artiarticle.objects.filter(published=True)[0]
        # next_article = Artiarticle.objects.filter(id=pk, published=True)[0]
    belong = {'name': '文章', 'slug': 'article'}
    column_tmp = Articleclass.objects.filter(slug=column)[0].name
...
def article(request, column):
    ...
    article = Artiarticle.objects.filter(column=column, published=True).order_by("-id")
    classes = Articleclass.objects.all()
    belong = 'article'
...
def index(request):
    tmpurl = str(request.path).strip('/')
    article = Artiarticle.objects.filter(published=True).order_by("-id")
    classes = Articleclass.objects.all()
    belong = 'article'
    objects, page_range = my_pagination(request, article, 15)
