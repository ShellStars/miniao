...
from .models import Recommendarticle, Recommendclass
...
def article_detail(request, column, pk):
    ...
    relate = Recommendarticle.objects.filter(column=column, published=True)[0:2]
    pre_article = Recommendarticle.objects.filter(id__lt=pk, published=True)
    if pre_article:
        pre_article = pre_article.order_by('-id')[0]
    else:
        pre_article = Recommendarticle.objects.filter(id=pk, published=True)[0]
    next_article = Recommendarticle.objects.filter(id__gt=pk, published=True)
    if next_article:
        next_article = next_article[0]
    else:
        next_article = Recommendarticle.objects.filter(published=True)[0]
        # next_article = Recommendarticle.objects.filter(id=pk, published=True)[0]
    belong = {'name': '药械推荐', 'slug': 'recommend'}
    column_tmp = Recommendclass.objects.filter(slug=column)[0].name
