...
from .models import Standardarticle, Resourcearticle
...
def download_detail(request, column, pk):
    article = Standardarticle.objects.filter(pk=pk, column=column, published=True)
    if article:
        num = article[0].browser + 1
        article.update(browser=num)
...
def downloadarticle(request, column):
    tmpurl = str(request.path).strip('/')
    article = Resourcearticle.objects.filter(column=column, published=True)
    if article:
        objects, page_range = my_pagination(request, article, 2)
        return render_to_response('article1.html', {'objects':objects,'page_range':page_range, 'tmpurl':tmpurl},context_instance=RequestContext(request))
    else:
...
def index(request):
    article = Standardarticle.objects.filter(column='gonggao', published=True)[0:3]
    if article:
