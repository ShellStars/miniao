...
from .models import Nursingarticle
...
def article(request, column):
    ...
    article = Nursingarticle.objects.filter(column=column, published=True)
    if article:
...
def index(request):
    article = Nursingarticle.objects.filter(column='gonggao', published=True)[0:3]
    if article:
