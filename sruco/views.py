...
from .models import Srucoarticle
...
def article(request, column):
    ...
    article = Srucoarticle.objects.filter(column=column, published=True)
    if article:
...
def index(request):
    article = Srucoarticle.objects.filter(column='gonggao', published=True)[0:3]
    if article:
