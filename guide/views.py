...
from .models import Guidearticle, Guideclass
...
def article_detail(request, column, pk):
    ...
    relate = Guidearticle.objects.filter(column=column, published=True)[0:2]
    pre_article = Guidearticle.objects.filter(id__lt=pk, published=True)
    if pre_article:
        pre_article = pre_article.order_by('-id')[0]
    else:
        pre_article = Guidearticle.objects.filter(id=pk, published=True)[0]
    next_article = Guidearticle.objects.filter(id__gt=pk, published=True)
    if next_article:
